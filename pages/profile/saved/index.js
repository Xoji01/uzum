...
async function fetchData() {
    try {
        ...
        if (!data || !data.favorites) {
            console.error('Ошибка: Данные не найдены в файле db.json');
            return;
        }

        const favorites = data.favorites.filter(item => item.userId === userId).map(item => item.product);
...
function displayFavorites(favorites) {
    ...
    if (favoritesContainer) {
        renderProducts(favorites, '.products');
    } else {
        console.error('Ошибка: Элемент с классом .favorites не найден на странице');
    }
}
...
function renderProducts(products, containerSelector) {
    ...
    if (!productsContainer) {
        console.error(`Ошибка: Элемент с селектором ${containerSelector} не найден на странице`);
        return;
    }
