...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('../../db.json')
        ...
        .then(data => {
            ...
            if (filteredGoods) {
                ...
                const filteredGoodsBySearch = goods.filter(item =>
                    item.title.toLowerCase().includes(searchTerm)
                );
                renderProducts(filteredGoodsBySearch);
            } else {
    ...
    const handleSearch = () => {
        ...
        if (!isNaN(priceMin)) {
            filteredGoods = filteredGoods.filter(item => item.price >= priceMin);
        }
        if (!isNaN(priceMax)) {
            filteredGoods = filteredGoods.filter(item => item.price <= priceMax);
        }
        renderProducts(filteredGoods);
    };
    ...
    priceMinInput.addEventListener('input', handleSearch);
    priceMaxInput.addEventListener('input', handleSearch);

    document.getElementById('forhomee').addEventListener('click', () => {
        const filteredGoods = goods.filter(item => ['kitchen', 'tv', 'PC'].includes(item.type));
        localStorage.setItem('filteredGoods', JSON.stringify(filteredGoods));
        renderProducts(filteredGoods);
    });

    document.getElementById('furnituree').addEventListener('click', () => {
        const filteredGoods = goods.filter(item => ['tv', 'kitchen'].includes(item.type));
        localStorage.setItem('filteredGoods', JSON.stringify(filteredGoods));
        renderProducts(filteredGoods);
    });

    document.getElementById('salee').addEventListener('click', () => {
        const filteredGoods = goods.filter(item => item.salePercentage > 0);
        localStorage.setItem('filteredGoods', JSON.stringify(filteredGoods));
        renderProducts(filteredGoods);
    });
});
